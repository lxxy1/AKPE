...
int Graph::ColorSort(vector<int> p, int &colorsetnum, vector<int> &colornode, bool f)
{
	high_resolution_clock::time_point Color_s = high_resolution_clock::now();
	colorsetnum = 0;
	colornode.clear();
	colornode.resize(NODENUM, -1);
	for (int i = 0; i < p.size();i++)
	{
		colorrecord.assign(colorrecord.size(), 0);
		if(f)
		{
			for (int j = 0; j < nlist[p[i]].Pdegree;j++)
			{
				int a = Padjncy[nlist[p[i]].Pid + j];
				if(colornode[a]!=-1)
				{
					colorrecord[colornode[a]] = 1;
				}
		...
		else
		{
			...
			for (int j = 0; j < nlist[p[i]].Ndegree;j++)
			{
				int a = Nadjncy[nlist[p[i]].Nid + j];
				if(colornode[a]!=-1)
				{
					colorrecord[colornode[a]] = 1;
				}
		...
		for (int r = 0; r < colorrecord.size();r++)
		{
			if(colorrecord[r] == 0)
			{
				colornode[p[i]] = r;
	...
	for(int i = 0; i < p.size();i++)
	{
		colorrecord[colornode[p[i]]]++;
	}
	int colord = 0;
	for (int i = 0; i < colorsetnum;i++)
	{
		colord += min(colorrecord[i], K);
	}
    high_resolution_clock::time_point Color_e = high_resolution_clock::now();
	colortime += std::chrono::duration_cast<std::chrono::microseconds>(Color_e - Color_s).count();
	return colord;
}

int Graph::ColorCheck(int a, int colorsetnum, vector<int> &colornode, bool f) // true is positive,false is all
{
	high_resolution_clock::time_point ColorCheck_s = high_resolution_clock::now();
	int cnum = 0;
	cn.assign(cn.size(), 0);
	for(int i = 0;i < nlist[a].Pdegree; i++)
	{
		int nowid = Padjncy[nlist[a].Pid + i];
		if(colornode[nowid] != -1 && colornode[nowid] != colornode[a])
		{
			cn[colornode[nowid]]++;
		}
	}
	if(!f)
	{
		for(int i = 0;i < nlist[a].Ndegree; i++)
		{
			int nowid = Nadjncy[nlist[a].Nid + i];
			if(colornode[nowid] != -1 && colornode[nowid] != colornode[a])
			{
				cn[colornode[nowid]]++;
			}
	...
	high_resolution_clock::time_point ColorCheck_e = high_resolution_clock::now();
	colorchecktime += std::chrono::duration_cast<std::chrono::microseconds>(ColorCheck_e - ColorCheck_s).count();
	return cnum;
}
...
void KPlexEnumUtil(Graph *subg, vector<int> CL, vector<int> CR, vector<int> PL, vector<int> PR, vector<int> QL, vector<int> QR, vector<int> CLdegree, vector<int> CRdegree, int level)
{
	...
	if (CL.size() + PLnew.size() < T || CR.size() + PRnew.size() < T)
		return;
	int PLcolorsetnum=0;
	vector<int> PLcolornode;
	int PRcolorsetnum=0;
	vector<int> PRcolornode;
	vector<int> A;
	int Acolorsetnum=0;
	vector<int> Acolornode;
    vectors_union(PLnew, PRnew, A);
	if(level<lnum)
    { 
        int PLcolor = subg->ColorSort(PLnew, PLcolorsetnum, PLcolornode, true) + CL.size();
        if (PLcolor < T)
        {
            
            return;
        }
        int PRcolor = subg->ColorSort(PRnew, PRcolorsetnum, PRcolornode, true) + CR.size();
        if (PRcolor < T)
        {
        ...
        int Acolor = subg->ColorSort(A, Acolorsetnum, Acolornode, false) + CL.size() + CR.size();
        if (Acolor < 2 * T)
        {
            return;
        }
	...
	vector<int> C;
	vectors_union(CL, CR, C);
	if (flag)
	{
		int pivot = -1;
		if (PLnew.size() != 0)
		{
			vector<int>::iterator ip = PLnew.begin();
			pivot = (*ip);
			vector<int> ipn;
			subg->vectorneighbor(pivot, ipn);
			vector<int> pi1;
			subg->pivotselect(PLnew, ipn, CL, pivot, true, pi1);
			vector<int> pi2;
			subg->pivotselect(PLnew, ipn, CR, pivot, false, pi2);
			vector<int> z1;
			vectors_difference(PLnew, ipn, z1);
			vector<int> z2;
			vectors_intersection(pi1, pi2, z2);
			vector<int> p;
			vectors_union(z1, z2, p);
			for (vector<int>::iterator i = p.begin(); i != p.end(); i++)
			{
                int ithis;
				ithis = (*i);
				for (vector<int>::iterator ii = PLnew.begin(); ii != PLnew.end(); ii++)
				{
					if ((*ii) == ithis)
					{
						PLnew.erase(ii);
                ...
                if(level<lnum)
                {
                    if (subg->ColorCheck((*i), PLcolorsetnum, PLcolornode, true) + K - subg->gvectors_difference(CL, (*i),true, emptyv) + CL.size() < T)
                    {
                        continue;
                    }
                    if (subg->ColorCheck((*i), Acolorsetnum, Acolornode, false) + K - subg->gvectors_difference(C, (*i), emptyv) + C.size() < 2 * T)
                    {
                        continue;
                    }
                }
		...
		if (PRnew.size() != 0)
		{
			...
			{
				subg->vectorneighbor(pivot, ipn);
				subg->pivotselect(PRnew, ipn, CR, pivot, true, pi1);
				subg->pivotselect(PRnew, ipn, CL, pivot, false, pi2);
				vector<int> z1;
				vectors_difference(PRnew, ipn, z1);
				vector<int> z2;
				vectors_intersection(pi1, pi2, z2);
				vectors_union(z1, z2, p);
			}
			...
			{
				vector<int>::iterator ip = PRnew.begin();
				pivot = (*ip);
				subg->vectorneighbor(pivot, ipn);
				subg->pivotselect(PRnew, ipn, CR, pivot, true, pi1);
				subg->pivotselect(PRnew, ipn, CL, pivot, false, pi2);
			...
			for (vector<int>::iterator i = p.begin(); i != p.end(); i++)
			{
                int ithis;
				ithis = (*i);
				for (vector<int>::iterator ii = PRnew.begin(); ii != PRnew.end(); ii++)
				{
					if ((*ii) == ithis)
					{
						PRnew.erase(ii);
                ...
                if(level<lnum)
                {
                    if (subg->ColorCheck((*i), PRcolorsetnum, PRcolornode, true) + K - subg->gvectors_difference(CR, (*i),true, emptyv) + CR.size() < T)
                    {
                        continue;
                    }
                    if (subg->ColorCheck((*i), Acolorsetnum, Acolornode, false) + K - subg->gvectors_difference(C, (*i), emptyv) + C.size() < 2 * T)
